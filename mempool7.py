...
for i, h in enumerate(HALVINGS):
    ...
    # 1. Update: Look further back to capture the Anticipatory Run-up
    baseline_start = h_block - (2 * EPOCH)
    anticipation_start = h_block - EPOCH
    daa_block = h_block + (EPOCH - (h_block % EPOCH))
    
    csv_filename = f"halving_{year}_{h_block}_extended_data.csv"
    ...
    if os.path.exists(csv_filename):
        ...
        data = fetch_blocks(baseline_start, daa_block, f"Fetching {year}")
        df = pd.DataFrame(data)
        df = df.sort_values('height').reset_index(drop=True)
        df['delta_t'] = df['timestamp'].diff().clip(lower=0) / 60.0 # Convert to minutes here
        df.to_csv(csv_filename, index=False)
        
    df_clean = df.dropna(subset=['delta_t']).copy()
    
    # --- PHASE SEPARATION ---
    phase_base = df_clean[(df_clean['height'] >= baseline_start) & (df_clean['height'] < anticipation_start)]['delta_t']
    phase_anticip = df_clean[(df_clean['height'] >= anticipation_start) & (df_clean['height'] < h_block)]['delta_t']
    phase_quench = df_clean[(df_clean['height'] >= h_block) & (df_clean['height'] < daa_block)]['delta_t']

    # --- METRICS ---
    mean_base, var_base = phase_base.mean(), phase_base.var(ddof=1)
    mean_ant, var_ant = phase_anticip.mean(), phase_anticip.var(ddof=1)
    mean_q, var_q = phase_quench.mean(), phase_quench.var(ddof=1)
    
    # Printouts for your analysis
    print(f"\n--- {year} Halving ---")
    print(f"Baseline Phase     (T-4032 to T-2016): Mean = {mean_base:.2f} min, Var = {var_base:.2f}")
    print(f"Anticipation Phase (T-2016 to T-0)   : Mean = {mean_ant:.2f} min, Var = {var_ant:.2f}")
    print(f"Quench Phase       (T-0 to DAA)      : Mean = {mean_q:.2f} min, Var = {var_q:.2f}")
    
    results.append({
        "Year": year,
        "Mean_Base": mean_base, "Var_Base": var_base,
        "Mean_Anticip": mean_ant, "Var_Anticip": var_ant,
        "Mean_Quench": mean_q, "Var_Quench": var_q
    })
    ...
    df_clean['rolling_mean'] = df_clean['delta_t'].rolling(window=window, center=True).mean()
    df_clean['rolling_std'] = df_clean['delta_t'].rolling(window=window, center=True).std()
    
    ax.plot(df_clean['relative_height'], df_clean['rolling_mean'], color='#1f77b4', linewidth=2, label=r'$\langle \Delta t \rangle$ (144-block avg)')
    ax.fill_between(df_clean['relative_height'], 
                    df_clean['rolling_mean'] - df_clean['rolling_std'], 
                    df_clean['rolling_mean'] + df_clean['rolling_std'], 
                    color='#1f77b4', alpha=0.2)
                    
    # Lines
    ax.axvline(-EPOCH, color='#ff7f0e', linestyle='-.', linewidth=2, label='Anticipation Start')
    ax.axvline(0, color='#d62728', linestyle='--', linewidth=2, label='Quench (Halving)')
    ax.axvline(daa_block - h_block, color='#2ca02c', linestyle=':', linewidth=2, label='DAA Intervenes')
    ...
    ax.set_ylabel('Inter-block time (minutes)')
    ax.set_ylim(0, 35) 
    ax.set_xlim(-EPOCH * 1.5, EPOCH * 1.2) # Adjusted limits to show anticipation
    if i == 0:
        ax.legend(loc='upper left')

plt.tight_layout()
plt.savefig('fig_halvings_dynamics_revtex_updated.pdf', format='pdf', bbox_inches='tight')
print("\n[INFO] Chart saved as 'fig_halvings_dynamics_revtex_updated.pdf'")