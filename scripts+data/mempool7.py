...
# --- CONFIGURATION GRAPHIQUE POUR PUBLICATION (RevTeX / APS) ---
plt.rcParams.update({
    "font.family": "serif",
    "font.serif": ["Times New Roman"],
    "axes.labelsize": 12,
...
# --- PARAMÈTRES ---
EPOCH = 2016
MAX_ANTICIPATION_EPOCHS = 14 # Test jusqu'à ~6.5 mois en arrière
BASELINE_DURATION = 6 * EPOCH # On fixe une baseline de 3 mois avant l'anticipation pour comparer
...
try:
    ...
    print(f"[{len(df)} blocs chargés et traités avec succès.]\n")
except FileNotFoundError:
    print(f"ERREUR : Fichier {MASTER_CSV_FILE} introuvable.")
    exit()

fig, axes = plt.subplots(2, 2, figsize=(14, 10))
axes = axes.flatten()

print("=== ANALYSE DE SENSIBILITÉ DE L'HORIZON D'ANTICIPATION ===")
...
for i, h in enumerate(HALVINGS):
    ...
    # 1. BALAYAGE (SWEEP) DES EPOCHS D'ANTICIPATION
    for k in range(1, MAX_ANTICIPATION_EPOCHS + 1):
        ...
        # Calcul du Delta Variance
        delta_var_pct = ((var_q - var_ant) / var_ant) * 100
        
        # Tests Statistiques (Anticipation vs Quench)
        ...
        # Sauvegarder si c'est le signal de Quench le plus fort (p_fisher minimum)
        if p_fisher < best_p_fisher and delta_var_pct > 0: # On cherche une explosion de variance
            best_p_fisher = p_fisher
            optimal_epochs = k
            
    print(f"--> Horizon Optimal Sélectionné pour le graphique : {optimal_epochs} Epochs (~{optimal_epochs * 2} semaines)")

    # 2. GÉNÉRATION DU GRAPHIQUE AVEC L'HORIZON OPTIMAL
    ...
    ax.set_title(f'Halving {year} (Horizon Optimal: {optimal_epochs} ep)', fontweight='bold')
    ax.set_xlabel('Blocs relatifs au Halving ($z$)')
    ax.set_ylabel('Temps inter-bloc (min)')
    ax.set_ylim(0, 35) 
...
print("\n[INFO] Graphique sauvegardé sous 'fig_halvings_sensitivity_revtex.pdf'")